...
<form id="form-tlist" method="POST" ftype="<?= $form_type ?>">
    <div class="row <?= (($form_type == 'Edit') ? '' : '') ?>">
        ...
        <div class="form-group mb-1 <?= (($form_type == 'Edit') ? 'col-lg-8 mx-4' : '') ?> d-flex align-items-center justify-content-end">
            <?= (($form_type == 'Edit') ? '' : '<a id="btn-close" class="bc"><i class="fas fa-close fs-5 me-4 text-secondary"></i></a>') ?><button type="<?= (($form_type == 'Edit') ? 'button' : 'submit') ?>" id="btn-upt" class="btn btn-primary" <?= (($form_type == 'Edit') ? 'style="display: none;"' : '') ?>><span><?= (($form_type == 'Edit') ? 'Update' : 'Save') ?></span></button>
        </div>
</form>
<!-- Comment Form -->
<?php if ($form_type == 'Edit') { ?>
    <form id="form-comment" method="POST">
        ...
        <div class="form-group col-lg-8 mx-3 mt-2 d-flex justify-content-between">
            <img src="<?= base_url('public/assets/images/faces/avatar-1.png') ?>" class="rounded-circle" width="35" height="35">
            <input type="hidden" id="task" name="task" value="<?= $row['id'] ?>">
            <textarea class="form-control form-control-sm" spellcheck="false" name="comment-input" id="comment-input" placeholder="Write a comment"></textarea>
        </div>
        <div class="form-group col-lg-8 d-flex justify-content-end mx-3 mb-1">
            <button type="button" id="btn-com" class="btn btn-inverse-primary">Send</button>
        </div>
    </form>
<?php } ?>
<?php if ($form_type == 'Edit') { ?>
