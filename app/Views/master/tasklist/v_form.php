<form id="form-tlist" method="POST" ftype="<?= $form_type ?>">
    <div class="row <?= (($form_type == 'Edit') ? 'm-1' : '') ?>">
        <div class="form-group <?= (($form_type == 'Edit') ? 'col-lg-8' : '') ?>">
            ...
            <input type="text" class="form-control form-control-sm mt-2 rounded" name="taskname" id="taskname" placeholder="Enter new task" value="<?= (($form_type == 'Edit') ? $row['tasklistname'] : '') ?>">
        </div>
        <?php if ($form_type == 'Edit') { ?>
            <div class="form-group col-lg-8">
                ...
                <textarea rows="10" cols="6" class="form-control form-control-sm mt-2 pt-3 pb-3 rounded" name="desc" id="desc" placeholder="Task Description"><?= (($form_type == 'Edit') ? $row['description'] : '') ?></textarea>
            </div>
        <?php } ?>
        <div class="form-group mb-1 <?= (($form_type == 'Edit') ? 'col-lg-8' : '') ?> d-flex align-items-center justify-content-end">
        ...
        <?php if ($form_type == 'Edit') { ?>
            ...
            <div class="col-lg-8 d-flex justify-content-between m-2">
                <img src="<?= base_url('public/assets/images/faces/avatar-1.png') ?>" class="rounded-circle me-2" width="40" height="40">
